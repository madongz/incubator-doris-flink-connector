...
import static org.apache.doris.flink.sink.ResponsePattern.COMMITTED_PATTERN;
import static org.apache.doris.flink.sink.ResponsePattern.LABEL_EXIST_PATTERN;
...
public class DorisStreamLoad implements Serializable {
    ...
    public void writeRecord(byte[] row) throws IOException{
        if(loadBatchFirstRecord) {
            loadBatchFirstRecord = false;
        } else {
            recordStream.write(lineDelimiter);
        }
        recordStream.write(row);
    }
    ...
    private void abortTransaction(long txnID) throws Exception {
        ...
        if(FAIL.equals(res.get("status"))) {
            Matcher m = COMMITTED_PATTERN.matcher(res.get("msg"));
            if(m.matches()) {
